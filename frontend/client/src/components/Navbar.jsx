...
export default function Navbar({ user, setUser }) {
  ...
  // ✅ Nav links — desktop + mobile dono mein
  const navLinks = (
    <>
      <Link to="/standup/new"
      ...
      {/* ✅ Profile — mobile menu mein */}
      <Link to="/profile"
        onClick={() => setMenuOpen(false)}
        className="text-gray-600 dark:text-gray-300 hover:text-purple-700 dark:hover:text-purple-400 transition text-sm py-2 md:py-0 md:hidden">
        👤 My Profile
      </Link>
    </>
  )
  ...
  const NotificationDropdown = ({ isMobile = false }) => (
    <div className={`absolute right-0 top-11 ${isMobile ? 'w-72' : 'w-80'} bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-xl shadow-lg z-50 overflow-hidden`}>
      ...
      <div className={`${isMobile ? 'max-h-60' : 'max-h-72'} overflow-y-auto`}>
  ...
  return (
    <nav className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700 px-4 md:px-6 py-3 transition-colors duration-200 relative z-40">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-4 text-sm">
          ...
          {/* ✅ Profile Avatar + Name + Logout — desktop  visible */}
          <div className="flex items-center gap-2">
            <Link to="/profile"
              ...
              {user?.avatar ? (
                <img
                  src={user.avatar}
                  alt={user.name}
                  className="w-7 h-7 rounded-full object-cover border-2 border-purple-200 dark:border-purple-700"
                />
              ) : (
            ...
            <button onClick={handleLogout}
              className="bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-300 px-3 py-1 rounded-full text-xs hover:bg-purple-200 dark:hover:bg-purple-800 transition whitespace-nowrap">
              Logout
            </button>
          </div>
        </div>
        ...
        <div className="flex md:hidden items-center gap-2">
          ...
          <div className="relative">
            ...
            {showDropdown && <NotificationDropdown isMobile />}
          </div>
          ...
          >
            <span className={`block w-4 h-0.5 bg-gray-600 dark:bg-gray-300 transition-all duration-200 ${menuOpen ? 'rotate-45 translate-y-2' : ''}`} />
            <span className={`block w-4 h-0.5 bg-gray-600 dark:bg-gray-300 transition-all duration-200 ${menuOpen ? 'opacity-0' : ''}`} />
            <span className={`block w-4 h-0.5 bg-gray-600 dark:bg-gray-300 transition-all duration-200 ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
          </button>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
            ...
            >
              {user?.avatar ? (
                ...
                <div className="w-8 h-8 rounded-full bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-300 flex items-center justify-center text-xs font-bold">
                  {user?.name?.charAt(0).toUpperCase()}
                </div>
              )}
            ...
            {/* ✅ Logout — mobile menu  clearly visible */}
            <button onClick={handleLogout}
              className="bg-red-50 dark:bg-red-950 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 px-4 py-1.5 rounded-full text-xs font-medium hover:bg-red-100 dark:hover:bg-red-900 transition">
              Logout
            </button>
          </div>
        </div>
